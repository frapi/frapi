...
class Default_Model_Action extends Lupin_Model
{
    ...
    public function add(array $data)
    {
        ...
        $data['name'] = Frapi_FileHelper::normalizeName($data['name']);
    ...
    public function sync()
    {
        ...
        foreach ($actions as $key => $a) {
            ...
            $name = Frapi_FileHelper::normalizeName($a['name']);
    ...
    {
        $className = 'Action_' . Frapi_FileHelper::generateClassName($name);
    ...
    private function createDirs($dir, $name)
    {
        $currdir = $dir;
        $expName = explode('/', $name);
        array_pop($expName);
        if ($expName && is_array($expName)) {
            foreach($expName as $diraddition) {
                $currdir .= DIRECTORY_SEPARATOR . $diraddition;
                mkdir($currdir);
            }
        }
    }
    ...
    private function generateAction($name, $properties, $description = false, $route = false)
    {
        ...
        $class->setName('Action_' . Frapi_FileHelper::generateClassName($name));
        ...
        $classDocblock = new Zend_CodeGenerator_Php_Docblock(array(
                'shortDescription' => 'Action ' . Frapi_FileHelper::generateClassName($name) . ' ',
